...
See [below](#datafile-conversions) for compatibility info.
...
The [`conversions/`](conversions/) directory contains scripts to convert known JSON datafiles so that they can be used with this tool.

- For datafiles generated by this tool before 2026: [`from_legacy.py`](conversions/from_legacy.py).
- For datafiles generated by [kryptogramm](https://github.com/infoaed/kryptogramm/tree/main): [`from_kryptogramm`](conversions/from_kryptogramm.py).
  
  Note that if the JSON name contains characters that are not URL safe (`+`, `/`), then this tool will not work with them.
  To fix this, replace `+` -> `-` and `/` -> `_`.
